...
import argparse,csv,json,threading,time,sys,math
from pathlib import Path
import cv2
from flask import Flask,Response,jsonify,request
from vision import Tracker

PAGE='''<!doctype html><title>Pendulum tracker</title><h2>Pendulum camera proof of concept</h2><p>Click the BLACK PIVOT SCREW at the narrow end in the image. Then let the arm hang down and set zero. Selecting a pivot clears the old zero.</p><button onclick="fetch('/home',{method:'POST'}).then(r=>r.json()).then(x=>alert(JSON.stringify(x)))">Set hanging zero</button> <button onclick="fetch('/clear',{method:'POST'})">Clear calibration</button><p><img id="feed" src="/stream" style="max-width:100%"></p><pre id="s"></pre><script>document.getElementById('feed').onclick=(e)=>{let im=e.target,r=im.getBoundingClientRect();fetch('/pivot',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({x:(e.clientX-r.left)/r.width,y:(e.clientY-r.top)/r.height})}).then(r=>r.json()).then(x=>alert(JSON.stringify(x)))};setInterval(()=>fetch('/state').then(r=>r.json()).then(x=>document.getElementById('s').textContent=JSON.stringify(x,null,2)),250)</script>'''
...
class Camera:
    def __init__(self,a):
        ...
        if not self.cap.isOpened():raise RuntimeError('Cannot open camera. On Mac try --camera 0 or 1 and allow Terminal camera access in System Settings. On Linux check /dev/video*. Close other camera apps.')
        if sys.platform!='darwin':self.cap.set(cv2.CAP_PROP_FOURCC,cv2.VideoWriter_fourcc(*a.fourcc))
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--camera',default='0' if sys.platform=='darwin' else '/dev/video0');p.add_argument('--width',type=int,default=960);p.add_argument('--height',type=int,default=600);p.add_argument('--fps',type=int,default=30);p.add_argument('--fourcc',default='MJPG');p.add_argument('--bind',default='127.0.0.1');p.add_argument('--port',type=int,default=3080);p.add_argument('--config',default='tracker.json');p.add_argument('--preview-fps',type=float,default=15);p.add_argument('--out',default='vision_runs');a=p.parse_args()
    if len(a.fourcc)!=4 or a.preview_fps<=0:p.error('fourcc must have 4 characters; preview fps must be positive')
    cfg=json.loads(Path(a.config).read_text());cfg.update(home_angle_rad=None,home_length_px=None);tracker=Tracker(cfg);cam=Camera(a);lock=threading.Lock();shared={'state':{'valid':False},'jpg':None};app=Flask(__name__)
        ...
        with (out/'angles.csv').open('w',newline='') as f:
            ...
            while not cam.stop:
                ...
                if start-preview>=1/a.preview_fps:
                    ...
                    if state['pivot_px'] is not None and state['tip_px'] is not None:cv2.line(display,tuple(map(round,state['pivot_px'])),tuple(map(round,state['tip_px'])),(0,255,255),2)
                    ...
                    label=f"down {state['theta_down_rad']*180/3.14159265:+.1f} deg" if state['valid'] else 'Select pivot / set hanging zero / tracking lost'
    ...
    def pivot():
        item=cam.get()
        if item is None or time.monotonic()-item[1]>.3:return jsonify(error='No fresh camera frame'),409
        body=request.get_json(silent=True) or {}
        try:x=float(body['x']);y=float(body['y'])
        except (KeyError,ValueError,TypeError):return jsonify(error='Invalid coordinates'),400
        if not (math.isfinite(x) and math.isfinite(y) and 0<=x<=1 and 0<=y<=1):return jsonify(error='Click inside image'),400
        h,w=item[2].shape[:2]
        with lock:
            tracker.select_pivot(x*w,y*h)
            with (out/'calibration_events.jsonl').open('a') as ef:ef.write(json.dumps(dict(t=time.monotonic(),event='pivot_selected',x=x*w,y=y*h))+'\n')
        return jsonify(ok=True,message='Pivot selected; let arm hang down then set zero')
    @app.post('/home')
    def home():
        with lock:
