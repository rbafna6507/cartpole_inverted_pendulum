...
import cv2
import numpy as np
from green_arm import GreenArm
...
class Tracker:
    def __init__(self,cfg):
        self.cfg=cfg;self.history=deque(maxlen=cfg['home_samples']);self.previous=None;self.detector=GreenArm(cfg) if cfg.get('mode')=='green_arm' else None
    def select_pivot(self,x,y):
        self.detector.select(x,y);self.history.clear();self.previous=None
        self.cfg.update(home_angle_rad=None,home_length_px=None)
    def home(self):
        if len(self.history)<self.history.maxlen:raise ValueError('Hold hanging still with the arm and pivot visible until 30 valid frames accumulate')
    ...
    def process(self,frame,t):
        if self.detector:p,q=self.detector.detect(frame,t)
        else:p=blob(frame,self.cfg['pivot_hsv'],self.cfg);q=blob(frame,self.cfg['tip_hsv'],self.cfg)
