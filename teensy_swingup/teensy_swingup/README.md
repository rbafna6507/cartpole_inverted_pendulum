...
- J7-only signal wiring below. Encoder uses Wire1 on GPIO17/16. No microstep-control GPIOs; Big Easy Driver onboard pull-ups retain 1/16 stepping.
...
The eight original controller/helper headers are unchanged byte for byte. Swing-up, capture, balance, rail/spin recovery, estimator, tuning, command names, and CRC16 output format are retained. Startup remains disabled, centered at the physical position at reset; no automatic homing is added.
...
Leave all driver MS1/MS2/MS3 pins unconnected. Remove their GPIO traces and selection jumpers from the planned PCB. The firmware no longer configures or writes microstep pins. Keep 1/16 in the mechanics: removing microstep control does not select full stepping. GPIO16/17 are now reserved for encoder I2C; GPIO20/24/25/26/27/28/29 are released. J6 is unused by the current wiring, and J5 has no project connections.
